...
const ControlRegister = ({flow,editable,handleUpdate}) => {
    ...
    const getAccion = (accion) => {
      ...
      const getAccionAprobacion = (accion) => {
        const map = {
            mensaje: {
            text: 'Mensaje de confirmación - via correo '
          },
          noMensaje: {
            text: 'No enviar mensaje'
          }
        };
      ...
      const getAccionRechazo = (accion) => {
        const map = {
            reenviar: {
            text: 'Realizar nuevamente el formulario'
          },
          mensaje: {
            text: 'Enviar mensaje de rechazo - via correo'
          },
          noMensaje: {
            text: 'No enviar mensaje'
          }
                ...
                <Box display={"flex"} flexDirection={"row"} >
                    ...
                    <Box display={"flex"} flexDirection={"column"} >
                        ...
                        <Autocomplete
                            disablePortal
                            disabled
                                value={getAccionAprobacion(flow.accionAprobacion)}
                            options={[
                        ...
                        <Autocomplete
                            disablePortal
                            disabled
                            value={getAccionRechazo(flow.accionAprobacion)}
                            options={[
